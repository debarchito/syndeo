        ...
        if (event.record.room === data.payload!.room.id) {
          if (event.action === "create") {
            const newMessage = {
              ...
              user: {
                name:
                  event.record.expand?.from_teacher?.name || data.payload?.user?.name || "Unknown",
...
<svelte:head>
  ...
  {:else}
    <title>
      {data.payload!.room.expand!.appointee.displayName ||
        data.payload!.room.expand!.appointee.name} | syndeo.
    </title>
  {/if}
</svelte:head>
      ...
      <div class="mx-auto flex max-w-4xl items-center justify-between p-3 sm:p-4">
        <div class="flex items-center gap-2 sm:gap-3">
          ...
          <div>
            ...
            >
              {data.payload!.room.expand!.appointee.displayName ||
                data.payload!.room.expand!.appointee.name}
            </h1>
        ...
        {#if !allMessages.length}
          ...
          >
            ...
            <p class="text-muted-foreground max-w-md text-xs sm:text-sm">
              Start a conversation with {data.payload!.room.expand!.appointee.displayName ||
                data.payload!.room.expand!.appointee.name}!
            </p>
          </div>
        {:else}
          ...
          >
            <Chat.List class="space-y-2 p-2 sm:space-y-3 sm:p-3 md:space-y-4 md:p-4">
              ...
              {#each allMessages as msg, index (msg.id + "-" + index)}
                {@const isSentByMe = data.payload?.user && msg.from_user === data.payload.user.id}
                {@const username = isSentByMe
                  ? data.payload?.user?.name
                  : msg.expand?.from_teacher?.name || data.payload!.room.expand!.appointee.name}
                <Chat.Bubble
      ...
      <div class="mx-auto max-w-4xl">
        {#if data.payload?.user}
          ...
          >
            ...
            <input
              type="hidden"
              name="to_teacher"
              value={data.payload!.room.expand!.appointee.id}
            />
        ...
        {:else}
          ...
          >
            ...
            <Button
              onclick={() =>
                goto(`/sign-in?redirect-to=/dashboard/message/room/${data.payload?.room.id}`)}
